...
if (isset($_GET['lor'])) {
    ...
    $lor = $_GET['lor'];
    $lor = explode(";", $lor); // checking input for a ";"
    if (count($lor) == 1) {
        // parameter without a ";", checking for url-encoded alias of ";"
        $pos = strpos("%3B", intval($lor));
        if ($pos === true) {
...
print '<html><head><title>Die '.$lor_names['lor_name'].' LOR-Seite</title>'
        ...
        .'<script src="/seiten/libs/raphael-min.js"></script>'
        .'<script src="/seiten/libs/g.raphael-min.js"></script>'
        .'<script src="/seiten/libs/g.line-min.js"></script>'
        .'<script src="/seiten/libs/jquery-1.11.3.min.js"></script>'
        .'<script src="/seiten/libs/OpenLayers.js"></script>'
        ...
        .'<script>'
            ...
            .'window.onload = function () {'
                ...
                .'setupMapNavigation("2017/06", lor_id);'
            .'}'
        .'</script>'

        .'<script src="/seiten/lor-page-diagram.js"></script>'

      .'</head><body>';

/* print '<div class="navigation">'
        .'<a href="#citymap-navigation" title="zur Navigation &uuml;ber die Karte">Kartennavigation</a>'
    .'</div>';**/
...
function render_migration_table($dataset) {
    ...
    $tab1 .= '<h3>1. Tabelle: Melderechtlich registrierte EinwohnerInnen nach Migrationshintergrund* '
        .'(Datenstand: Juni 2017)<a name="migration">&nbsp;</a></h3>';
    ...
    $tab1 .= '<tr class="average"><td>Gesamtzahl der Bewohner</td><td>'.$inhabitants_overall.'</td>'
        ...
        .'Amt f&uuml;r Statistik Berlin-Brandenburg</a>, Abgestimmter Datenpool Juni 2017<br/></td></tr>';
    $tab1 .= '</table>';
...
function render_citizen_table($dataset) {
 ...
 // get data
    ...
    $table = "";
    $table .= '<h3>2. Tabelle: Staatsangeh&ouml;rigkeit ausgew&auml;hlter L&auml;nder (Datenstand: Juni 2017)'
        .'<a name="staaten">&nbsp;</a></h3>';
...
function render_age_table($dataset, $lor_names, $id) {
    ...
    $tab1 = "";
    $tab1 .= '<h3>3. Tabelle: Altersverteilung "'.$lor_names['lor_name'].'", Berlin-'.$lor_names['district'].' (Datenstand: Juni 2017) '
    ...
    $tab1 .= '<div class="diagram-layer-controls">'
        ...
        .'<span title="Vergleichen sie die aktuellen Werte mit dem Stand Juni 2014" '
            . 'onclick="update_diagram(this, 201406, '.'\''.$id.'\''.');">Juni 2014</span>&nbsp;'
        .'<span title="Vergleichen sie die aktuellen Werte mit dem Stand Juni 2015" '
            . 'onclick="update_diagram(this, 201506, '.'\''.$id.'\''.');">Juni 2015</span>&nbsp;'
        .'<span title="Vergleichen sie die aktuellen Werte mit dem Stand Dez. 2015" '
            . 'onclick="update_diagram(this, 201512, '.'\''.$id.'\''.');">Dez. 2015</span>&nbsp;'
        .'<span title="Vergleichen sie die aktuellen Werte mit dem Stand Juni 2016" '
            . 'onclick="update_diagram(this, 201606, '.'\''.$id.'\''.');">Juni 2016</span>&nbsp;'
        .'<span title="Vergleichen sie die aktuellen Werte mit dem Stand Dezember 2016" '
            . 'onclick="update_diagram(this, 201612, '.'\''.$id.'\''.');">Dez. 2016</span>&nbsp;'
        .'</div>';
...
function get_lor_migration_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWRMIGRA201706_1_Kiezatlas.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);

    // return values
    $lorow = array();
    $overall = array('ger_without_migration' => 0.0, 'ger_without_migration_avg' => 0.0, 'ger_with_migration' => 0.0, 'ger_with_migration_avg' => 0.0, 'migrants' => 0.0, 'migrants_avg' => 0.0, );
    $line = 0;
    $startLine = 1;
...
function get_lor_citizen_row($id) {
    ...
    // implementation partially copied from http://stackoverflow.com/questions/5299471/php-parsing-a-txt-file
    $txt_file    = file_get_contents('data/EWR_AUSL_Staat201706.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
    ...
    $overall = array('eu-05-avg' => 0.0, 'eu-15-avg' => 0.0, 'eu-poland-avg' => 0.0, 'eu-2007-avg' => 0.0, 'eu-2013-avg' => 0.0,
        'ex-yugoslavia-avg' => 0.0, 'ex-sowjetunion-avg' => 0.0, 'turkyie-avg' => 0.0, 
        'arabic_countries-avg' => 0.0, 'other_countries-avg' => 0.0, 'not_sure-avg' => 0.0, 
...
function get_lor_age_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWR201706E_Matrix.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
    // return values
    $lorow = array();
    $overall = array('e_m-avg' => 0.0, 'e_w-avg' => 0.0, '00-01-avg' => 0.0, '01-02-avg' => 0.0, '02-03-avg' => 0.0, 
        ...
        '45-50-avg' => 0.0, '50-55-avg' => 0.0, '55-60-avg' => 0.0, '60-63-avg' => 0.0, '63-65-avg' => 0.0, '65-67-avg' => 0.0,
        '67-70-avg' => 0.0, '70-75-avg' => 0.0, '75-80-avg' => 0.0, '80-85-avg' => 0.0, '85-90-avg' => 0.0, '90-95-avg' => 0.0,
        '95-110-avg' => 0.0);
    ...
    foreach($rows as $row => $data) {
        ...
        // Note: row data seperated by semicolon!
        $row_data = explode(';', $data);
...
function render_monitoring_data($dataset) {
    return '<h3>Sozialdaten aus dem Monitoring Soziale Stadtentwicklung 2010<a name="monitoring">&nbsp;</a></h3>'
        ...
        .'Referat IA) - &copy; f&uuml;r diese Auswertung GskA, Projekt Network (2011-2017)</div><br/><br/>';
}
...
function render_social_atlas_data($dataset) {
    return '<h3>Sozialdaten aus dem Sozialstrukturatlas 2009<a name="sozialstrukturatlas">&nbsp;</a></h3>'
        ...
        .'&copy; f&uuml;r diese Auswertung GskA, Projekt Network (2011-2017)</div><br/><br/>';
}

function render_history_list($id) {
    return '<div class="footer">'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2016/12" class="current" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2016/12/?lor='.$id.'">12/2016 (HTML)</a>'
		.'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2016/06" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2016/06/?lor='.$id.'">06/2016 (HTML)</a>'
		.'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2015/12" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2015/12/?lor='.$id.'">12/2015 (HTML)</a>'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2015/06" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2015/06/?lor='.$id.'">06/2015 (HTML)</a>'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2014/12" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2014/12/?lor='.$id.'">12/2014 (HTML)</a>'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2014/06" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2014/06/?lor='.$id.'">06/2014 (HTML)</a>'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2013/12" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2013/12/?lor='.$id.'">12/2013 (HTML)</a>'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2013/06" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2013/06/?lor='.$id.'">06/2013 (HTML)</a>'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2012/12" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2012/12/?lor='.$id.'">12/2012 (HTML)</a>'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2012/06" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2012/06/?lor='.$id.'">06/2012 (HTML)</a>'
        .'<a title="Diretlink: zu den Daten dieser LOR-Seite von 2011/12" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2011/12/?lor='.$id.'">12/2011 (HTML)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/analysen_06_2011/'.$id.'.pdf">06/2011 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/analysen_02_2011/'.$id.'.pdf">12/2010 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/analysen_2010/'.$id.'.pdf">2010 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/analysen_2009/'.$id.'.pdf">2009 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/analysen_2008/'.$id.'.pdf">2008 (PDF)</a></div>';
}
...
function render_city_navigation() {
    return '<h3 id="nav-header">Navigation und geografische Ansicht zu diesem Planungsraum</h3>'
        ...
        .' &nbsp;Erweiterte LOR-Geometrie (KML) basiert auf <a href="http://www.statistik-berlin-brandenburg.de/produkte/opendata/geometrienOD.asp?Kat=6301">07/2012</a> CC-BY,  Amt f&uuml;r Statistik Berlin-Brandenburg<br/></p><a name="citymap-navigation">&nbsp;</a>';
}
...
print '<h2>Migration, Staatsangeh&ouml;rigkeit und Altersverteilung<br/>31. Juni 2017</h2>';

print '<h4>Vergleichszahlen f&uuml;r diesen Planungsraum aus den anderen Erhebungszeitr&auml;umen finden Sie hier:</h4>';
...
print '<h4>Vergleichszahlen aus den anderen Erhebungszeitr&auml;umen finden Sie hier:</h4>';
...
print '<div class="footer">'
    .'&copy; 2011-2017 f&uuml;r diese Auswertung <a href="http://www.spinnenwerk.de/network/" '
