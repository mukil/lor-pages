...
function render_migration_table($dataset) {
    ...
    $tab1 .= '<h3>1. Tabelle: Melderechtlich registrierte EinwohnerInnen nach Migrationshintergrund* '
        .'(Datenstand: Dezember 2013)<a name="migration">&nbsp;</a></h3>';
...
function render_age_table($dataset, $lor) {
    ...
    $tab1 = "";
    $tab1 .= '<h3>3. Tabelle: Altersverteilung "'.$lor['lor_name'].'", Berlin-'.$lor['district'].' (Datenstand: Dezember 2013) '
...
function get_lor_migration_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWRMIGRA201312_1_Kiezatlas.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_citizen_row($id) {
    ...
    // implementation partially copied from http://stackoverflow.com/questions/5299471/php-parsing-a-txt-file
    $txt_file    = file_get_contents('data/EWR_AUSL_Staat201312_clean_head.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_age_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWR201312E_Matrix.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
    // return values
    $lorow = array();
