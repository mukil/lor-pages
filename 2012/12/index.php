...
print '<html><head><title>Die '.$lor_names['lor_name'].' LOR-Seite</title>'
        ...
        .'<script>'
            .'window.onload = function () {'
                ...
                .'setupMapNavigation("2012/12", lor_id);'
             .'}'
        .'</script>'
      .'</head><body>';
...
function render_migration_table($dataset) {
    ...
    $tab1 .= '<h3>1. Tabelle: Melderechtlich registrierte EinwohnerInnen nach Migrationshintergrund* '
        .'(Datenstand: Dezember 2012)<a name="migration">&nbsp;</a></h3>';
    ...
    $tab1 .= '<tr class="average"><td>Gesamtzahl der Bewohner</td><td>'.$inhabitants_overall.'</td>'
        ...
        .'Amt f&uuml;r Statistik Berlin-Brandenburg</a>, Abgestimmter Datenpool Dezember 2012<br/></td></tr>';
    $tab1 .= '</table>';
...
function render_citizen_table($dataset) {
    ...
    $table = "";
    $table .= '<h3>2. Tabelle: Staatsangeh&ouml;rigkeit ausgew&auml;hlter L&auml;nder (Datenstand: Dezember 2012)'
        .'<a name="staaten">&nbsp;</a></h3>';
...
function render_age_table($dataset, $lor) {
    ...
    $tab1 = "";
    $tab1 .= '<h3>3. Tabelle: Altersverteilung "'.$lor['lor_name'].'", Berlin-'.$lor['district'].' (Datenstand: Dezember 2012) '
...
function get_lor_migration_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWRMIGRA201212_1_Kiezatlas_cleaned_head.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_citizen_row($id) {
    ...
    // implementation partially copied from http://stackoverflow.com/questions/5299471/php-parsing-a-txt-file
    $txt_file    = file_get_contents('data/EWR_AUSL_Staat201212_cleaned_head.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_age_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWR201212E_Matrix.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
    // return values
    $lorow = array();
