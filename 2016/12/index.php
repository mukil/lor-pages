...
print '<html><head><title>Die '.$lor_names['lor_name'].' LOR-Seite</title>'
        ...
        .'<script>'
            ...
            .'window.onload = function () {'
                ...
                .'setupMapNavigation("2016/12", lor_id);'
            .'}'
        .'</script>'
...
function render_migration_table($dataset) {
    ...
    $tab1 .= '<h3>1. Tabelle: Melderechtlich registrierte EinwohnerInnen nach Migrationshintergrund* '
        .'(Datenstand: Dezember 2016)<a name="migration">&nbsp;</a></h3>';
    ...
    $tab1 .= '<tr class="average"><td>Gesamtzahl der Bewohner</td><td>'.$inhabitants_overall.'</td>'
        ...
        .'Amt f&uuml;r Statistik Berlin-Brandenburg</a>, Abgestimmter Datenpool Dezember 2016<br/></td></tr>';
    $tab1 .= '</table>';
...
function render_citizen_table($dataset) {
 ...
 // get data
    ...
    $table = "";
    $table .= '<h3>2. Tabelle: Staatsangeh&ouml;rigkeit ausgew&auml;hlter L&auml;nder (Datenstand: Dezember 2016)'
        .'<a name="staaten">&nbsp;</a></h3>';
...
function render_age_table($dataset, $lor_names, $id) {
    ...
    $tab1 = "";
    $tab1 .= '<h3>3. Tabelle: Altersverteilung "'.$lor_names['lor_name'].'", Berlin-'.$lor_names['district'].' (Datenstand: Dezember 2016) '
...
function get_lor_migration_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWRMIGRA201612_1_Kiezatlas.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_citizen_row($id) {
    ...
    // implementation partially copied from http://stackoverflow.com/questions/5299471/php-parsing-a-txt-file
    $txt_file    = file_get_contents('data/EWR_AUSL_Staat201612.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_age_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWR201612E_Matrix.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
    // return values
    $lorow = array();
...
print '<h2>Migration, Staatsangeh&ouml;rigkeit und Altersverteilung<br/>31. Dezember 2016</h2>';
