...
print '<html><head><title>Die '.$lor_names['lor_name'].' LOR-Seite</title>'
        ...
        .'<script>'
            ...
            .'window.onload = function () {'
                ...
                .'setupMapNavigation("2016/06", lor_id);'
            .'}'
        .'</script>'
...
function render_migration_table($dataset) {
    ...
    $tab1 .= '<h3>1. Tabelle: Melderechtlich registrierte EinwohnerInnen nach Migrationshintergrund* '
        .'(Datenstand: Juni 2016)<a name="migration">&nbsp;</a></h3>';
    ...
    $tab1 .= '<tr class="average"><td>Gesamtzahl der Bewohner</td><td>'.$inhabitants_overall.'</td>'
        ...
        .'Amt f&uuml;r Statistik Berlin-Brandenburg</a>, Abgestimmter Datenpool Juni 2016<br/></td></tr>';
    $tab1 .= '</table>';
...
function render_citizen_table($dataset) {
 ...
 // get data
    ...
    $table = "";
    $table .= '<h3>2. Tabelle: Staatsangeh&ouml;rigkeit ausgew&auml;hlter L&auml;nder (Datenstand: Juni 2016)'
        .'<a name="staaten">&nbsp;</a></h3>';
...
function render_age_table($dataset, $lor_names, $id) {
    ...
    $tab1 = "";
    $tab1 .= '<h3>3. Tabelle: Altersverteilung "'.$lor_names['lor_name'].'", Berlin-'.$lor_names['district'].' (Datenstand: Juni 2016) '
...
function get_lor_migration_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWRMIGRA2010616_1_Kiezatlas-rd.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_citizen_row($id) {
    ...
    // implementation partially copied from http://stackoverflow.com/questions/5299471/php-parsing-a-txt-file
    $txt_file    = file_get_contents('data/EWR_AUSL_Staat201606-rd.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
...
function get_lor_age_row($id) {
    ...
    $txt_file    = file_get_contents('data/EWR201606E_Matrix-rd.csv');
    $rows        = explode("\n", $txt_file);
    array_shift($rows);
    // return values
    $lorow = array();
...
function render_history_list($id) {
    return '<div class="footer">'
        .'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2016/12" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/2016/12/?lor='.$id.'">12/2016 (HTML)</a>'
		.'<a title="Direktlink: zu den Daten dieser LOR-Seite von 2016/06" class="current" '
