...
function render_migration_table($dataset) {
    ...
    $tab1 .= '<h3>1. Tabelle: Melderechtlich registrierte EinwohnerInnen nach Migrationshintergrund* '
        .'(Datenstand: Dezember 2014)<a name="migration">&nbsp;</a></h3>';
...
function render_citizen_table($dataset) {
    ...
    $table = "";
    $table .= '<h3>2. Tabelle: Staatsangeh&ouml;rigkeit ausgew&auml;hlter L&auml;nder (Datenstand: Dezember 2014)'
        .'<a name="staaten">&nbsp;</a></h3>';
...
function render_age_table($dataset, $lor_names, $id) {
    ...
    $tab1 = "";
    $tab1 .= '<h3>3. Tabelle: Altersverteilung "'.$lor_names['lor_name'].'", Berlin-'.$lor_names['district'].' (Datenstand: Dezember 2014) '
...
function get_lor_age_row($id) {
    ...
    foreach($rows as $row => $data) {
        ...
        // mark for later return the searched for lor-data-row
        if ($info[$row]['lor_id'] === $id) {
            $lorow = $info[$row];
        } else { // thanks to changed value-format we may fill-in the leading zeros to get a match ..
            $csv_id = '0'.$info[$row]['lor_id'];
            $proper_id = ''.$id;
            if ($csv_id === $proper_id) {
                $lorow = $info[$row];
            }
...
function render_monitoring_data($dataset) {
    return '<h3>Sozialdaten aus dem Monitoring Soziale Stadtentwicklung 2010<a name="monitoring">&nbsp;</a></h3>'
        ...
        .'Referat IA) - &copy; f&uuml;r diese Auswertung GskA, Projekt Network (2013-2015)</div><br/><br/>';
}
...
function render_social_atlas_data($dataset) {
    return '<h3>Sozialdaten aus dem Sozialstrukturatlas 2009<a name="sozialstrukturatlas">&nbsp;</a></h3>'
        ...
        .'&copy; f&uuml;r diese Auswertung GskA, Projekt Network (2013-2015)</div><br/><br/>';
}
...
function render_history_list($id) {
    return '<div class="footer">'
        ...
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/analysen_06_2011/'.$id.'.pdf">06/2011 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/analysen_02_2011/'.$id.'.pdf">12/2010 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/analysen_2010/'.$id.'.pdf">2010 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/analysen_2009/'.$id.'.pdf">2009 (PDF)</a>'
        .'<a title="Direktlink: PDF (~280 KByte)" alt="Direktlink: PDF (~280 KByte)" '
        .'href="http://sozialraumdaten.kiezatlas.de/seiten/analysen_2008/'.$id.'.pdf">2008 (PDF)</a></div>';
}
...
print '<div class="footer">'
    .'&copy; 2013-2015 f&uuml;r diese Auswertung <a href="http://www.spinnenwerk.de/network/" '
