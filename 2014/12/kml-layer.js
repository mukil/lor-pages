...
if ("onhashchange" in window) { // event supported?'
    window.onhashchange = function (anchor) {
        if (anchor.newURL.indexOf('#citymap-navigation')) {

            window.showMapNavigationArea();
            window.scrollY = window.scrollY + 400;
            // showMapElements();
            // setupMapNavigation();
...
function setupMapNavigation(date_string, lor_id) {
    ...
    var lor = new OpenLayers.Layer.Vector("KML Layer", {
        ...
        protocol: new OpenLayers.Protocol.HTTP({
            url: "/~lor/seiten/2012/06/data/LOR-Planungsraeume-rev2.kml", 
